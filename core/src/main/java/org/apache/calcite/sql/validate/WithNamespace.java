...
public class WithNamespace extends AbstractNamespace {
  ...
  protected void validateAlwaysFilterImpl(Set<String> alwaysFilterFields) {
    for (SqlNode withItem: with.withList) {
      ((AlwaysFilterValidator) validator).validateWithItemAlwaysFilter((SqlWithItem) withItem, alwaysFilterFields);
    }
    final SqlValidatorScope scope2 =
        validator.getWithScope(Util.last(with.withList));
    ((AlwaysFilterValidator) validator).validateQueryAlwaysFilter(with.body, scope2, alwaysFilterFields);
  }
